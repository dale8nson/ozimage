...
function Carousel({
  ...
  return (
    ...
    >
      <div className={cn('group/hover relative flex-col space-y-2', className)}>
        <div className='flex-col space-y-2 items-center overflow-hidden'>{children}</div>
      </div>
    </CarouselProvider>
  );
}
...
function CarouselIndicator({
  ...
  return (
    <div
      className={cn(
        'relative z-10 flex md:max-w-370 h-fit items-start',
        className
      )}
    >
      <div className='flex object-cover  md:max-w-370  max-h-[151px] overflow-hidden '>
        {images.map((img, i) => (
          <div key={i}  className="max-w-[211px] max-h-[151px] overflow-hidden cursor-pointer">
            {img && <NextImage
              ...
              className={cn(
                'h-full w-full transition-transform hover:scale-110 duration-300',
                // classNameButton
              )}
            />}
          </div>
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        !disableDrag && 'cursor-grab active:cursor-grabbing w-370',
        className
      )}
      ref={containerRef}
    >
...
function CarouselItem({ children, className }: CarouselItemProps) {
  return (
    <motion.div
      className={cn(
        'w-full min-w-0 shrink-0 overflow-hidden object-cover',
        className
      )}
    >
      {children}
