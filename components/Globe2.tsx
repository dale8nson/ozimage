...
import { Line as LineMesh, Sky, Stars, useGLTF } from '@react-three/drei';
...
export const Globe2 = ({ apiToken: token }: { apiToken: Promise<string | undefined> }) => {
  ...
  const dracoLoader = new DRACOLoader().setDecoderPath("/draco3d/")
  const apiToken = use(token) as string
  const assetId = 1
